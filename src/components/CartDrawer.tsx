    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full bg-card border-border">
        <SheetHeader className="flex-shrink-0">
          <SheetTitle className="font-heading text-2xl font-light text-foreground">Shopping Bag</SheetTitle>
            ...
            <>
              <div className="flex-1 overflow-y-auto pr-2 min-h-0">
                <div className="space-y-4">
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-3 border border-border rounded-sm bg-secondary/30">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-heading text-lg font-medium truncate text-foreground">{item.product.node.title}</h4>
                        <p className="text-sm text-muted-foreground">{item.selectedOptions.map(option => option.value).join(' • ')}</p>
                        <p className="font-medium text-primary mt-1">{item.price.currencyCode} {parseFloat(item.price.amount).toFixed(2)}</p>
                      </div>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t border-border bg-card">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-heading text-primary">{items[0]?.price.currencyCode || 'ZAR'} {totalPrice.toFixed(2)}</span>
                </div>
